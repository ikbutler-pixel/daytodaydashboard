...
async function readAll(query) {
  ...
  if (process.env.NOTION_MEALPLAN_DB) {
    ...
    // Two days back was enough when the dashboard only ever showed one day.
    // The 14-day plan grid can page backwards, and a window that starts at
    // today-2 renders those past days blank rather than showing what was
    // actually eaten. 60 days back is roughly four pages of history.
    mealsRaw = await queryAll(
      mealDs,
      { property: 'Date', date: { on_or_after: isoDaysFromNow(-60) } },
      [{ property: 'Date', direction: 'ascending' }]
    );
  }
